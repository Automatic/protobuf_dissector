...
function Decoder:popLimit(old_limit, old_cursor)
    dassert(old_limit >= self.limit, "Programming error: popLimit() called with smaller limit")
...
function Decoder:advance(size)
    ...
    dassert(self.limit >= 0, "Programming error: limit dropped below 0")
    self.cursor = self.cursor + size
    dassert(self.cursor <= (#self.raw + 1), "Programming error: cursor went beyond raw length")
end
...
function Decoder:addExpert(tree, expert_type, append_text)
    ...
    dassert(expert_fields[expert_type], "Programming error: invalid expert_type given:", expert_type)
...
function Decoder:addExpertTvb(tree, expert_type, start, stop, append_text)
    ...
    tree:add_tvb_expert_info(expert_fields[expert_type], self.tvbuf(start,stop), text)
...
function Decoder:decodeTags(tags, frequency)
    ...
    if frequency then
        local ok, expert_type, append_text = frequency:checkMessage()
        if not ok then
            self:addExpert(nil, expert_type, append_text)
        end
    end
...
function Decoder:addFieldInfo(tree, size)
    local tag, field_start, key_hdr_size, tvbuf = self.tag, self.field_start, self.key_hdr_size, self.tvbuf

    -- add the key field and subtree
    local key_tree = tree:add(pfields.key, tvbuf(field_start, key_hdr_size))
    ...
    tree:add(pfields.value, tvbuf(field_start + self.key_size, size))

    if self.frequency then
        local ok, expert_type, append_text = self.frequency:checkTag(tag)
        if not ok then
...
function Decoder:addFieldVarintSigned32(pfield)
    -- apparently protobuf encodes a negative int32 as a int64!
    return self:addFieldVarint(pfield, varint.decodeSigned32As64)
end


function Decoder:addFieldVarintZigZag32(pfield)
    return self:addFieldVarint(pfield, varint.decodeZigZag32)
end
...
function Decoder:addFieldVarintZigZag64(pfield)
    return self:addFieldVarint(pfield, varint.decodeZigZag64)
end
...
-- just adds a ProtoField of limit size, does not advance cursor this is meant
-- to be used with Message for its Proto, and also with Group which does not
-- know the size until later
function Decoder:addField(pfield, name)
    local tree = self.tree:add(pfield, self.tvbuf(self.field_start, self.limit + self.key_size))
    if name then
        tree:set_text(name)
    end
